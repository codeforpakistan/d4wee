...
def index(request):
    ...
    enrollments = Enrollment.objects.prefetch_related('course').filter(status=Enrollment.StatusChoices.IN_PROGRESS, registration__student__user=user, registration__cohort__status=Cohort.StatusChoices.ACTIVE)
