...
3. Check Google OAuth authentication:
   - Ensure the Google Social Account is still connected
   - May need to re-authenticate if tokens expired
   - Visit `/admin/socialaccount/socialaccount/` to verify

4. Verify the service can run:
   ```bash
   sudo systemctl start d4wee-sync.service
   sudo systemctl status d4wee-sync.service
   ```

### Testing Sync Locally (Development)

On a local Windows machine with `uv`:

```bash
# Navigate to project directory
cd D:\Desktop\Code4PK\UNWomen\D4WEE

# Run sync command
uv run python manage.py sync

# Run sync with clearing active cohort data first
uv run python manage.py sync --clear

# View cohort statistics after sync
